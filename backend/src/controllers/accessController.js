...
const solicitarPinCorreo = async (req, res) => {
    ...
    try {
        ...
        if (assignmentResult.rows.length === 0)
            return res.status(404).json({ mensaje: 'Sin casillero asignado.' });
        ...
        console.log(`[CORREO ENVIADO] PIN enviado a ${email}`);
...
const abrirLockerRemoto = async (req, res) => {
    ...
    try {
        ...
        const pinQuery = await db.query(
            "SELECT pin_hash, expires_at FROM pin_tokens WHERE assignment_id = $1", [assignment_id]
        );
        if (pinQuery.rows.length === 0)
            return res.status(400).json({ mensaje: "No has solicitado un PIN." });
